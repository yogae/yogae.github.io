...
coverage와 test를 하는 경우 `setup.cfg` file을 추가할 수 있습니다.
...
test를 실핼 하기 위해서는 `pytest` 명령을 사용합니다.

test의 code coverage를 측정하기 위해서는 `coverage` 명령을 사용합니다.

```bash
pytest # test 실행
coverage run -m pytest # code coverage 측정
coverage html # html file로 coverage resport를 만들어 줌 -> htmlcov/index.html에서 확인 가능
```
